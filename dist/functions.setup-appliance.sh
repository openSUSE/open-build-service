...
function check_server_cert {
  ...
  if [[ ! -e $backenddir/certs/server.${FQHOSTNAME}.created || ! -e $backenddir/certs/server.${FQHOSTNAME}.crt ]]; then
    ...
    if [ ! -e $backenddir/certs/server.${FQHOSTNAME}.crt ] ; then
      ...
      disable_ssl_for_mariadb_in_rails
      if [[ $? == 0 ]];then
...
###############################################################################
function disable_ssl_for_mariadb_in_rails() {
  [[ $BOOTSTRAP_TEST_MODE == 1 ]] && return
  initdir=$apidir/config/initializers
  [ -d $initdir ] || mkdir -p $initdir
  if [ ! -f $initdir/mariadb.rb ];then
    cat <<EOF > $initdir/mariadb.rb
# This file was auto-generated by setup-appliance.sh/obsapisetup.service
# Your changes will NOT be overwritten
# If you remove this file it may be re-created on the next run of setup-appliance.sh/obsapisetup.service
