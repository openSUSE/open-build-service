...
# 
#
class ProjectAddGroupPage < ProjectPage
  ...
  def validate_page
    super
    validate { @driver.page_source.include? "Add New Group to " + @project }
  ...
  def initialize web_driver, options={}
    super
    @url = $data[:url] + "/project/add_group?project=" + CGI.escape(@project)
  end
  ...
  def initialize_ready web_driver
    ...
    assert @url.start_with? $data[:url] + "/project/add_group?"
  end
  ...
  def add_group group, role, options = {}
    ...
    if options[:expect] == :success
      ...
      validate { flash_message == 
        "Added group #{group} with role #{role} to project #{@project}" }
