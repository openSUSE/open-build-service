  ...
  def show_status_comment( comment, package, firstfail, comments_to_clear )
    ...
    if comment
      ...
      if !firstfail
        ...
        status_comment_html += " "
        status_comment_html += link_to_remote image_tag('/themes/bento/images/icons/comment_edit.png', :alt => "Edit"), :update => "comment_edit_#{package.gsub(':', '-')}",
          :url => { :action => "edit_comment_form", :comment=> comment, :package => package, :project => @project }
      end 
    elsif firstfail
      if @project.is_maintainer?( session[:login] )
        status_comment_html += " <span class='unknown_failure'>Unknown build failure " + link_to_remote( image_tag('/themes/bento/images/icons/comment_edit.png', :size => "16x16", :alt => "Edit"),
