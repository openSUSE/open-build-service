#
# this plugin extends action caching mechanisms of rails, see:
#
# - http://wiki.rubyonrails.org/rails/pages/Action+Cache+Update+Plugin
# - http://craz8.com   (authors homepage)
# - http://blog.craz8.com   (authors blog)
# - http://craz8.com/svn/trunk/plugins/action_cache   (SVN)
#
  ...
  module Caching
    module Actions
      ...
      class ActionCachePath
        ...
        def path
          return @path if @path
          returning @path = @controller.send(:action_fragment_key, @options) do
            add_extension!
            clean!
          end
        end
      ...
      class ActionCacheFilter #:nodoc:
        ...
        def before(controller)
          if cache_entry = cached_entry(controller)
            if cache_entry.expired? || cache_data_missing?(controller)
      ...
      protected
        ...
        def send_cached_response(cache_entry, controller)
          ...
          controller.response.body = CGI.unescape(cache_entry.body)
        end
        ...
        def save_to_cache(controller)
          ...
          if split_cache?(controller)          
            controller.write_fragment(body_name(controller), controller.response.body)
          else
            cache.body = CGI.escape(controller.response.body)
          end
        ...
        def split_cache?(controller)
          x_sendfile_enabled?(controller) || x_accel_redirect_enabled?(controller)
        end
        
        def cache_data_missing?(controller)
          split_cache?(controller) ? !File.exists?(fragment_body_filename(controller)) : false
        end
