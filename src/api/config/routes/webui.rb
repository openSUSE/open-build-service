...
resources :projects, only: [], param: :name do
  ...
  resources :distros, controller: 'webui/distros', except: %i[index new], constraints: cons
end

resources :distros, only: [] do
  resources :releases, controller: 'webui/distro_releases', except: %i[index]
end
...
resource :labels, controller: 'webui/labels', only: %i[update], constraints: cons do
  collection do
    get 'autocomplete'
  end
end
