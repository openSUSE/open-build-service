RSpec.describe EventMailer, :vcr do
  ...
  describe '.notification_email' do
    ...
    context 'for an event of type Event::AddedGlobalRole' do
      ...
      let!(:subscription) { create(:event_subscription_added_global_role, user: staff_user) }
      ...
      before do
        login(admin)
        Event::AddedGlobalRole.create!(who: admin.login, user: user.login, role: 'Staff')
      end
      ...
      it 'sends an email to other staff users' do
        expect(mail.to).to include(staff_user.email)
      end

      it 'contains the correct subject' do
        expect(mail.subject).to include("'#{admin}' gave the 'Staff' role to the user '#{user}'")
      end

      it 'contains the correct text' do
        expect(mail.body.encoded).to include("'#{admin}' gave the 'Staff' role to the user '#{user}'")
      end
    end
