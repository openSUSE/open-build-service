...
RSpec.describe StagingProjectCopyJob, :vcr do
  include ActiveJob::TestHelper

  describe '#perform' do
    let(:user) { create(:confirmed_user, :with_home) }
    let(:staging_workflow) { create(:staging_workflow, project: user.home_project) }
    let!(:original_staging_project) do
      create(:staging_project, staging_workflow: staging_workflow, project_config: 'Prefer: something')
    end
    ...
    it 'copies the staging project' do
      ...
      StagingProjectCopyJob.perform_now(staging_workflow.project.name, original_staging_project.name,
                                        staging_project_copy_name, user.id)
