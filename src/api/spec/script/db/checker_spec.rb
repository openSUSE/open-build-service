...
RSpec.describe DB::Checker do
  ...
  describe '#constraints_to_check' do
    it { expect(checker.constraints_to_check).to be_a(Array) }
    it { expect(checker.constraints_to_check).not_to be_empty }

    it 'constraints are well defined' do
      checker.constraints_to_check.each do |constraint|
