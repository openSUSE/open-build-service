...
RSpec.describe PackageDatatable, type: :datatable do
  ...
  describe '#data' do
    it 'includes SCM indicator when package has scmsync' do
      ...
      expect(data.first[:name]).to include('fa-code-branch')
    end
    ...
    it 'includes Link indicator when package is a link' do
      package_link = create(:package, name: 'link_pkg', project: project)
      PackageKind.create!(package: package_link, kind: 'link')
      allow(datatable).to receive(:records).and_return([package_link])

      expect(datatable.data.first[:name]).to include('Link', 'fa-link')
    end
  end
end
