RSpec.describe Backend::ConnectionHelper do
  ...
  describe '#calculate_endpoint' do
    ...
    context 'with a template' do
      ...
      it {
        expect(subject.send(:calculate_endpoint,
                            ['/build/:param1/:param2', 'my_param',
                             'my_2nd_param'])).to eq('/build/my_param/my_2nd_param')
      }

      it {
        expect(subject.send(:calculate_endpoint,
                            ['/build/:param1/:param2', 'home:param2', 'blah'])).to eq('/build/home:param2/blah')
      }
    end
    ...
    context 'with a wrong formed template' do
      it {
        expect do
          subject.send(:calculate_endpoint, ['/build/:param'])
