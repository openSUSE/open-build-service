...
RSpec.describe 'MaintenanceWorkflow', js: true, vcr: true do
  ...
  let!(:maintenance_package) { create(:package_with_file, project: project, name: 'cacti') }
  ...
  it 'maintenance workflow without patchinfo' do
    ...
    # visit package_show_path(project: update_project, package: maintenance_package)
    ...
    target_packages = []
    user.run_as do
      target_packages << BranchPackage.new(project: project.name, package: maintenance_package.name, target_project: 'home:tom:branches:Update',
                                           target_package: 'cacti.openSUSE:11.4').branch
    end
    user.run_as do
      target_packages << BranchPackage.new(project: another_project.name, package: another_package.name, target_project: 'home:tom:branches:Update',
                                           target_package: 'cacti.openSUSE:11.5').branch
    end
    update_project_branch = Project.last
    target_packages.each do |package|
      Backend::Connection.put("/source/#{package[:data][:targetproject]}/#{package[:data][:targetpackage]}/DUMMY_FILE", 'dummy')
    end
