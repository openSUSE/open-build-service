...
RSpec.describe Package, :vcr do
  ...
  describe '.for_notifications_on_packages' do
    let(:user) { create(:confirmed_user, :with_home) }
    let(:other_user) { create(:confirmed_user) }
    let(:project) { user.home_project }
    let(:package) { create(:package, project: project) }
    let(:other_package) { create(:package, project: project) }

    context 'with a direct package notification' do
      let!(:notification) { create(:notification_for_package, :web_notification, :relationship_create_for_package, notifiable: package, subscriber: user) }

      it 'includes package notified for user' do
        expect(Package.for_notifications_on_packages(user)).to include(package)
      end

      it 'excludes package notified for other user' do
        expect(Package.for_notifications_on_packages(other_user)).not_to include(package)
      end

      it 'excludes package when web is false' do
        notification.update!(web: false)
        expect(Package.for_notifications_on_packages(user)).not_to include(package)
      end
    end

    context 'with a BsRequest where the package is the target' do
      let!(:bs_request) { create(:bs_request_with_submit_action, target_package: package) }
      let!(:notification) { create(:notification_for_request, :web_notification, :request_state_change, notifiable: bs_request, subscriber: user) }

      it 'includes package whose target request is notified for user' do
        expect(Package.for_notifications_on_packages(user)).to include(package)
      end

      it 'excludes package when notification belongs to other user' do
        expect(Package.for_notifications_on_packages(other_user)).not_to include(package)
      end
    ...
    context 'with a BsRequest where the package is the source' do
      let!(:bs_request) { create(:bs_request_with_submit_action, source_package: package) }
      let!(:notification) { create(:notification_for_request, :web_notification, :request_state_change, notifiable: bs_request, subscriber: user) }

      it 'includes package whose source request is notified for user' do
        expect(Package.for_notifications_on_packages(user)).to include(package)
      end
    ...
    context 'with a comment on the package' do
      let!(:comment) { create(:comment_package, commentable: package) }
      let!(:notification) { create(:notification_for_comment, :web_notification, :comment_for_package, notifiable: comment, subscriber: user) }

      it 'includes package with a notified comment for user' do
        expect(Package.for_notifications_on_packages(user)).to include(package)
      end
    ...
    context 'with a report on the package' do
      let!(:report) { create(:report, reportable: package) }
      let!(:notification) { create(:notification_for_report, :web_notification, :report_for_user, notifiable: report, subscriber: user) }

      it 'includes package with a notified report for user' do
        expect(Package.for_notifications_on_packages(user)).to include(package)
      end
