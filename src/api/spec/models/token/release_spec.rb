RSpec.describe Token::Release, :vcr do
  ...
  let(:project_staging) do
    create(:project_with_package, name: 'Bar:Staging', package_name: 'bar_package', maintainer: user)
  end
  let(:target_project) { create(:project, name: 'Bar', maintainer: user) }
  let(:package) { project_staging.packages.first }

  let(:repository) do
    create(:repository, name: 'package_test_repository', architectures: ['x86_64'], project: project_staging)
  end
  let(:target_repository) do
    create(:repository, name: 'target_repository', architectures: ['x86_64'], project: target_project)
  end
  ...
  describe '#call' do
    context 'when no release target is set' do
      it 'throws an exception and sets the triggered_at column' do
        expect do
          subject
        end.to raise_error(Token::Errors::NoReleaseTargetFound,
                           'Bar:Staging has no release targets that are triggered manually')
    ...
    context 'when a manual release target is set' do
      let!(:release_target) do
        create(:release_target, target_repository: target_repository, repository: repository, trigger: 'manual')
      end
      let(:backend_url) do
      ...
      it 'triggers the release process in the backend' do
        user.run_as do
          expect(subject.first).to have_attributes(repository_id: repository.id,
    ...
    context 'when the release target is provided through parameters' do
      ...
      let(:other_source_repository) do
        create(:repository, name: 'other_source_repository', architectures: ['x86_64'], project: project_staging)
      end
      let(:other_target_repository) do
        create(:repository, name: 'other_target_repository', architectures: ['x86_64'], project: other_target_project)
      end
      let(:backend_url) do
      ...
      let!(:release_target) do
        create(:release_target, target_repository: other_target_repository, repository: other_source_repository,
                                trigger: 'manual')
      end
      ...
      context 'when the target_project, targetrepository, filter_source_repository and arch parameters are provided' do
        subject do
          token.call(package: package, project: project_staging, targetproject: 'Baz',
                     targetrepository: 'other_target_repository', filter_source_repository: 'other_source_repository')
        end
      ...
      context 'when the user can not modify the target_repository' do
        ...
        let(:other_target_repository) do
          create(:repository, name: 'other_target_repository', project: other_target_project)
        end

        subject do
          token.call(package: package, project: project_staging, targetproject: 'Foo',
                     targetrepository: 'other_target_repository', filter_source_repository: 'other_source_repository')
        end
        ...
        it 'does not trigger the release process in the backend' do
          user.run_as do
            expect do
              subject
            end.to raise_error(Token::Errors::InsufficientPermissionOnTargetRepository,
                               'no permission to write in project Foo')
