RSpec.describe ProjectLinks do
  ...
  describe '#add_project_link' do
    context 'with a local project' do
      ...
      it 'creates a new LinkedProject' do
        expect { project.add_project_link(source_project_name: project_to_link_against.name) }.to change(LinkedProject, :count).by(1)
      end

      it 'links to the local project' do
        project.add_project_link(source_project_name: project_to_link_against.name)
      ...
      it 'appends subsequent links to the bottom of the list (handled by acts_as_list)' do
        ...
        project.add_project_link(source_project_name: project_to_link_against.name)
        project.add_project_link(source_project_name: another_project.name)
        expect(project.linking_to.last.position).to eq(2)
      end

      it 'does not create a duplicate link for the same project' do
        project.add_project_link(source_project_name: project_to_link_against.name)
        expect { project.add_project_link(source_project_name: project_to_link_against.name) }.not_to change(LinkedProject, :count)
      end
    end
    ...
    context 'with a remote project' do
      ...
      it 'creates a new LinkedProject' do
        expect { project.add_project_link(source_project_name: remote_project_name) }.to change(LinkedProject, :count).by(1)
      end

      it 'stores the remote project name' do
        project.add_project_link(source_project_name: remote_project_name)
    ...
    context 'with a non-existing project' do
      it 'fails with an exception' do
        expect { project.add_project_link(source_project_name: 'nonexisting') }.to raise_error(Project::Errors::UnknownObjectError)
      end
    end
  end
  ...
  describe '#remove_project_link' do
    context 'with a local project' do
      ...
      before do
        project.add_project_link(source_project_name: linked_project.name)
      end
