...
RSpec.describe Project, vcr: true do
  ...
  describe '#accept' do
    ...
    let(:target_project) { create(:project, name: 'target_project') }
    let(:source_project) { create(:project, :as_submission_source, name: 'source_project') }
    let(:target_package_1) { create(:package, name: 'target_package_1', project: target_project) }
    let(:source_package) { create(:package, name: 'source_package', project: source_project) }
    ...
    let(:staged_request) do
      create(
        ...
        state: :new,
        creator: requester,
        description: "Request for package #{target_package_1}",
        target_package: target_package_1,
        source_package: source_package,
    ...
    context 'when the staging project is not in state acceptable' do
      ...
      it { expect(staging_project.packages.count).to be 4 }
    ...
    context 'when the project is being merged already' do
      let!(:user_relationship) { create(:relationship, project: target_project, user: user) }
      ...
      it { expect(staging_project.overall_state).to eq(:accepting) }
      it { expect(staged_request_with_by_project_review.reload.state).not_to eq(:accepted) }
    end
  end
end
