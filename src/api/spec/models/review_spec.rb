  ...
  describe 'validations' do
    it 'is not allowed to specify by_user and any other entity' do
      [:by_group, :by_project, :by_package].each do |entity|
        review = Review.new
        review[entity] = 'not-existent-entity'
        review[:by_user] = user.login
        review.save
  ...
  describe '.set_associations' do
    context 'with valid attributes' do
      ...
      it 'sets package and project associations when by_package and by_project object exists' do
        review = Review.create(by_project: project.name, by_package: package.name)
        expect(review.valid?).to eq(true)
        expect(review.entity).to eq(package)
        expect(review.by_package).to eq(package.name)
        expect(review.entity.project).to eq(project)
        expect(review.by_project).to eq(project.name)
      end
    end
