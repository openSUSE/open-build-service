...
RSpec.describe Review do
  ...
  describe '#maintainers_and_reviewers_for_package' do
    let(:review_by_user) { create(:review, by_user: user.login) }
    let(:package) { create(:package) }

    context 'when the review has a package' do
      let(:other_user) { create(:user, login: 'bob') }

      context 'when the package has relationships' do
        let(:maintainer) { Role.hashed['maintainer'] }

        context 'when the relationship belongs to a user as a maintainer or a reviewer' do
          let(:review_by_user) { create(:review, by_user: user.login) }

          before do
            package.relationships.create(user: other_user, role: maintainer)
            review_by_user.package = package
          end

          it 'returns a user' do
            expect(review_by_user.maintainers_and_reviewers_for_package).to contain_exactly(other_user)
          end
        end

        context 'when the relationship belongs to a user not being a maintainer nor a reviewer' do
          ...
          it 'returns an empty collection' do
            expect(review_by_user.maintainers_and_reviewers_for_package).to be_empty
          end
        end

        context 'when the relationship belongs to a group as a maintainer or reviewer' do
          ...
          it 'returns the user belonging to the group' do
            expect(review_by_user.maintainers_and_reviewers_for_package).to contain_exactly(other_user)
          end
        end
      end

      context 'when the package does not have relationships' do
        before do
          review_by_user.package = package
        end
    ...
    context 'when the review does not have a package' do
      it 'returns an empty collection' do
        expect(review_by_user.maintainers_and_reviewers_for_package).to be_empty
      end
    end
  ...
  describe '#maintainers_and_reviewers_for_project' do
    let(:review_by_user) { create(:review, by_user: user.login) }

    subject { review_by_user.maintainers_and_reviewers_for_project }

    context 'when the review has a package' do
      context 'when the package has a project' do
        let(:project) { create(:project_with_package) }
        let(:package) { project.packages.first }

        context 'when the project has relationships' do
          let(:other_user) { create(:user, login: 'bob') }
        ...
        context 'when the project does not have relationships' do
          before do
            review_by_user.package = package
          end
      ...
      context 'when the package does not have a project' do
        let(:package) { create(:package) }
