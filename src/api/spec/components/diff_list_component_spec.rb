RSpec.describe DiffListComponent, :vcr, type: :component do
  ...
  let(:bs_request_opts) { { filelimit: nil, tarlimit: nil, superseded_request: nil, diffs: true, cacheonly: 1 } }
  ...
  context 'with a request having a submit action' do
    before do
      ...
      render_inline(described_class.new(diff_list: action[:sourcediff].first['files']))
    end

    it 'renders the diff header' do
      expect(rendered_content).to have_text(file_name)
      expect(rendered_content).to have_text('Changed')
    end
