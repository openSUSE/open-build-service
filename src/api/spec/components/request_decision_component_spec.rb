RSpec.describe RequestDecisionComponent, type: :component do
  ...
  let(:project_with_devel) { create(:project, name: 'project_with_devel', maintainer: maintainer) }
  let(:package_with_devel) { create(:package, name: 'package_with_devel', project: project_with_devel, develpackage: devel_package) }
  ...
  context 'when we can forward the request to the developed project' do
    ...
    let!(:another_action) do
      ...
      another_action.target_package_object = package_with_devel
      another_action.save!
      another_action
    end
