RSpec.describe Webui::Packages::TriggerController, :vcr do
  ...
  describe 'POST #services' do
    ...
    context 'success' do
      before do
        allow(Backend::Api::Sources::Package).to receive(:trigger_services)
    ...
    context 'when triggering services in manual mode fails' do
      ...
      before do
        allow(Backend::Api::Sources::Package).to receive(:trigger_services).and_raise(Backend::Error.new('services are in manual mode!'))
        post :services, params: { project_name: project, package_name: package }
      end
    ...
    context 'when triggering services fails' do
      ...
      before do
        allow(Backend::Api::Sources::Package).to receive(:trigger_services).and_raise(Backend::Error.new('no source service defined!'))
        post :services, params: { project_name: project, package_name: package }
      end
