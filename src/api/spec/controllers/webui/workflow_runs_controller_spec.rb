  ...
  describe 'GET #index' do
    ...
    context 'when action_filter is available' do
      it 'finds workflow runs when the action is available' do
        get :index,
            params: { token_id: workflow_token.id, request_action: 'opened', generic_event_type: 'pull_request' }
      ...
      it 'does not find workflow runs when action is not available' do
        get :index,
            params: { token_id: workflow_token.id, request_action: 'closed', generic_event_type: 'pull_request' }
