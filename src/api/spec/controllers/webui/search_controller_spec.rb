RSpec.describe Webui::SearchController, :vcr do
  ...
  describe 'GET #owner' do
    ...
    it 'assigns results' do
      ...
      stub_request(:post,
                   "#{CONFIG['source_url']}/search/published/binary/id?match=(@name='package'%20and%20(@project='home:Iggy'))").and_return(body: collection)
    ...
    context 'with search_text starting with obs://' do
      context 'and a package' do
        before do
          ...
          get :index,
              params: { search_text: "obs://build.opensuse.org/#{user.home_project.name}/i586/1-#{package.name}" }
        end

        it {
          expect(subject).to redirect_to(controller: :package, action: :show, project: user.home_project, package: package.name,
                                         rev: 1)
        }
      end
      ...
      context 'and a non existent package' do
        before do
          ...
          get :index,
              params: { search_text: "obs://build.opensuse.org/#{user.home_project.name}/i586/1-non_existent_package" }
        end
