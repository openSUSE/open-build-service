RSpec.describe SourcePackageCommandController, :vcr do
  ...
  describe 'POST #undelete' do
    subject { post :undelete, params: parameters.merge({ cmd: 'undelete', project: project, package: package, format: :xml }) }

    let(:parameters) { {} }
    let(:project) { create(:project, name: 'franz', maintainer: user) }
    let(:package) { create(:package, name: 'hans', project: project) }

    context 'without permissions to undelete the package' do
      before do
        project.relationships.delete_all
        package.destroy
        login user
      end
    ...
    context 'without permissions to set the time' do
      let(:parameters) { { time: DateTime.now.next_week.to_i } }
