...
class BsRequestTest < ActiveSupport::TestCase
  ...
  def test_create_no_project
    ...
    #TODO assert_equal "Unknown source project home:guest", req.check_create(@tom)

    req = BsRequest.find(:name => "no_such_project2")
    #TODO assert_equal "Unknown target project openSUSE:Factory", req.check_create(@tscholz)
  end

  def test_create_no_package
    req = BsRequest.find(:name => "no_such_package")
    #TODO assert_equal "Unknown source package mypackage in project home:tscholz", req.check_create(@tom)
  end

  def test_create_works
    req = BsRequest.find(:name => "works")
    #TODO assert_equal "No permission to create request for package 'TestPack' in project 'home:tscholz'", req.check_create(@tom)
    #TODO assert_equal nil, req.check_create(@tscholz)
  end

  def test_oldformat
    req = BsRequest.find(:name => "oldformat")
    req2 = BsRequest.find(:name => "newformat")
    #TODO assert_equal req2.dump_xml, req.dump_xml
  end
