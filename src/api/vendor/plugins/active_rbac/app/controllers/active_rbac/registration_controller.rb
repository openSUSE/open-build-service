...
class ActiveRbac::RegistrationController < ActiveRbac::ComponentController
  ...
  def register
    if request.method != :post
      ...
      ActiveRbacConfig.config(:controller_registration_signup_fields).each do |field|
        field[:validation_proc].call
      end

      @user = User.new(params[:user])
      @user.password_hash_type = ActiveRbacConfig.config(:model_default_hash_type)
    ...
    @additional_partials = ActiveRbacConfig.config(:controller_registration_signup_fields).collect do |field|
      field[:template_path]
    end
  end
