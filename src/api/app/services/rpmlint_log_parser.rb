class RpmlintLogParser
  ...
  def parse_line(line, index)
    ...
    result = { location: line_m[:packagearch], level: line_m[:level], linter: line_m[:linter], badness: 0, line: index + 1 }
    result[:repo] = @repo if @repo
    result[:arch] = @arch if @arch
    ...
    when 'E'
      ...
      badness_value = ::Regexp.last_match[:badness].to_i if line_m[:rest] =~ /\(Badness: (?<badness>\d+)\)/
      badness[package] += badness_value || 1
      result[:badness] = badness_value || 1
    when 'W'
      warnings[package] += 1
    when 'I'
      info[package] += 1
