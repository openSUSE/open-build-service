class DiffParser
  ...
  def extract_diff_content(block, state)
    block.map { |l| l.state == state ? l.content[1..] : '' }.join
  end
  ...
  def inline_diff(removed_string, added_string)
    ...
    diff.each do |action, content|
      ...
      parsed_content = action == :equal ? content : content.lines(chomp: true).map { |d| "<span class=\"inline-diff\">#{d}</span>" }.join("\n")
