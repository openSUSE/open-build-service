module Backend
  ...
  module ConnectionHelper
    ...
    def http_get(endpoint, options = {})
      Backend::Connection.get(calculate_url(endpoint, options), String.new(calculate_headers(options).body, encoding: 'UTF-8'))
    end

    # Performs a http post request to the configured OBS Backend server.
    # @return [String]
    def http_post(endpoint, options = {})
      Backend::Connection.post(calculate_url(endpoint, options), options[:data], String.new(calculate_headers(options).body, encoding: 'UTF-8'))
    end

    # Performs a http put request to the configured OBS Backend server.
    # @return [String]
    def http_put(endpoint, options = {})
      Backend::Connection.put(calculate_url(endpoint, options), options[:data], String.new(calculate_headers(options).body, encoding: 'UTF-8'))
    end

    # Performs a http delete request to the configured OBS Backend server.
    # @return [String]
    def http_delete(endpoint, options = {})
      Backend::Connection.delete(calculate_url(endpoint, options), String.new(calculate_headers(options).body, encoding: 'UTF-8'))
    end
