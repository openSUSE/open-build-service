...
builder.staging_project(attributes) do
  if options[:requests]
    builder.staged_requests(count: staging_project.staged_requests.count) do
      render(partial: 'staging/shared/requests',
             locals: { requests: staging_project.staged_requests, builder: builder })
    end
    builder.untracked_requests(count: staging_project.untracked_requests.count) do
      render(partial: 'staging/shared/requests',
             locals: { requests: staging_project.untracked_requests, builder: builder })
    end
    builder.obsolete_requests(count: staging_project.staged_requests.obsolete.count) do
      render(partial: 'staging/shared/requests',
             locals: { requests: staging_project.staged_requests.obsolete, builder: builder })
    end
  ...
  if options[:history]
    builder.history(count: staging_project.project_log_entries.where(event_type: %i[staged_request
                                                                                    unstaged_request]).count) do
