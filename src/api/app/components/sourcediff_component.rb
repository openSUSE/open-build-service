class SourcediffComponent < ApplicationComponent
  ...
  def diff_list(sourcediff)
    ...
    files.each_with_index do |(filename, _contents), file_index|
      files[filename]['diff_url'] = request_changes_file_path(number: @bs_request.number, request_action_id: @action.id, filename:, diff_to_superseded:, file_index:, commented_lines: @commented_lines[file_index])
    end
