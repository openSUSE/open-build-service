ThinkingSphinx::Index.define :project, :with => :active_record, :delta => true do
  ...
  has "(SELECT max(activity_index) FROM packages WHERE "\
        "db_project_id = projects.id "\
        "AND NOT EXISTS (SELECT * FROM packages p WHERE p.db_project_id = projects.id AND p.updated_at > packages.updated_at))",
  ...
  has "EXISTS (SELECT * FROM packages INNER JOIN packages p ON p.id = packages.develpackage_id WHERE p.db_project_id = projects.id)", :as => :is_devel, :type => :boolean
  has "(SELECT max(updated_at) FROM packages WHERE db_project_id = projects.id)", :as => :updated_at, :type => :timestamp
end
