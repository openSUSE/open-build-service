class Webui::RequestController < Webui::WebuiController
  ...
  def changes_diff
    ...
    if @action.source_package_object&.file_exists?(filename, { rev: source_rev, expand: 1 }.compact)
      source_file = project_package_file_path(project_name: @action.source_project, package_name: @action.source_package, filename: filename, rev: source_rev, expand: 1)
    end
    target_rev = sourcediff.dig('old', 'srcmd5')
    if @action.target_package_object&.file_exists?(filename, { rev: target_rev, expand: 1 }.compact)
      target_file = project_package_file_path(project_name: @action.target_project, package_name: @action.target_package, filename: filename, rev: target_rev, expand: 1)
    end
