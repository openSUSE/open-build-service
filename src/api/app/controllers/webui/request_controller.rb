class Webui::RequestController < Webui::WebuiController
  ...
  before_action :require_login,
                except: %i[show sourcediff diff request_action request_action_changes inline_comment build_results
                           rpm_lint changes mentioned_issues]
  ...
  before_action :set_active_action, only: %i[inline_comment show build_results rpm_lint changes mentioned_issues],
                                    if: -> { Flipper.enabled?(:request_show_redesign, User.session) }
  before_action :set_superseded_request,
                only: %i[show request_action request_action_changes build_results rpm_lint changes mentioned_issues]
  ...
  def show
    ...
    if Flipper.enabled?(:request_show_redesign, User.session)
      ...
      @actions = @bs_request.webui_actions(filelimit: @diff_limit, tarlimit: @diff_limit,
                                           diff_to_superseded: @diff_to_superseded, diffs: false)
      ...
      @can_add_reviews = @bs_request.state.in?(%i[new
                                                  review]) && (@is_author || @is_target_maintainer || @my_open_reviews.present?)
  ...
  def create
    ...
    # FIXME: Use validations in the model instead of raising whenever something is wrong
      ...
      flash[:error] =
        "Unable to submit: The source of package #{elide(params[:project_name])}/#{elide(params[:package_name])} is broken"
  ...
  def request_action
    ...
    if @action[:diff_not_cached]
      ...
      job = Delayed::Job.where('handler LIKE ?',
                               "%job_class: BsRequestActionWebuiInfosJob%#{bs_request_action.to_global_id.uri}%").count
  ...
  def set_bugowner_request
    ...
    begin
      ...
      flash[:error] = e.message
      if params[:package]
        redirect_to(controller: :package, action: :show, package: params[:package],
                    project: params[:project]) && return
      end
  ...
  def build_results
    unless @bs_request_action.tab_visibility.build
      redirect_to request_show_path(params[:number],
                                    params[:request_action_id])
    end
  ...
  def rpm_lint
    unless @bs_request_action.tab_visibility.rpm_lint
      redirect_to request_show_path(params[:number],
                                    params[:request_action_id])
    end
  ...
  def changes
    unless @bs_request_action.tab_visibility.changes
      redirect_to request_show_path(params[:number],
                                    params[:request_action_id])
    end
  ...
  def mentioned_issues
    unless @bs_request_action.tab_visibility.issues
      redirect_to request_show_path(params[:number],
                                    params[:request_action_id])
    end
  ...
  def complete_build_results
    ...
    render partial: 'webui/request/beta_show_tabs/build_status',
           locals: { build_results_data: build_results_data, bs_request_number: @bs_request.number,
                     filters: filters }
  end
  ...
  def check_beta_user_redirect
    redirect_to request_show_path(params[:number], params[:request_action_id]) unless Flipper.enabled?(
      :request_show_redesign, User.session
    )
  end
  ...
  def set_superseded_request
    ...
    flash[:error] =
      "Request #{params[:diff_to_superseded]} does not exist or is not superseded by request #{@bs_request.number}."
    nil
  end
  ...
  def forward_request_to(fwd)
    ...
    begin
      forwarded_request = @bs_request.forward_to(project: tgt_prj, package: tgt_pkg,
                                                 options: params.slice(:description))
    rescue APIError, ActiveRecord::RecordInvalid => e
    ...
    target_link = ActionController::Base.helpers.link_to("#{tgt_prj} / #{tgt_pkg}",
                                                         package_show_url(project: tgt_prj, package: tgt_pkg))
    request_link = ActionController::Base.helpers.link_to("request #{forwarded_request.number}",
                                                          request_show_path(forwarded_request.number))
  ...
  def set_supported_actions
    ...
    @supported_actions = @actions.where(type: %i[add_role change_devel delete submit maintenance_incident
                                                 maintenance_release set_bugowner])
  end
  ...
  def prepare_request_data
    ...
    @issues = (@action.fetch(:sourcediff, []) || []).reduce({}) do |accumulator, sourcediff|
      accumulator.merge(sourcediff.fetch('issues', {}))
    end
