  ...
  module Groups
    class BsRequestsController < WebuiController
      ...
      def incoming_query
        @bs_requests.where(
          bs_request_actions: {
            target_project_id: @group.relationships.projects.maintainers.pluck(:project_id),
            target_package_id: @group.relationships.packages.maintainers.pluck(:package_id)
          }
        )
      end
      ...
      def outgoing_query
        @bs_requests.where(
          bs_request_actions: {
            source_project_id: @group.relationships.projects.maintainers.pluck(:project_id),
            source_package_id: @group.relationships.packages.maintainers.pluck(:package_id)
          }
        )
      end

      def review_query
        @bs_requests.where(reviews: { group_id: @group.id }).or(
          @bs_requests.where(
            reviews: {
              project_id: @group.relationships.projects.maintainers.pluck(:project_id),
              package_id: @group.relationships.packages.maintainers.pluck(:package_id)
            }
          )
        )
      end
