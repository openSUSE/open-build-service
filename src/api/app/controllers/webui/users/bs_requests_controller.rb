    ...
    class BsRequestsController < WebuiController
      ...
      def incoming_query
        ...
        @bs_requests.where(bs_request_actions: { target_project_id: user.relationships.projects.maintainers.pluck(:project_id),
                                                 target_package_id: user.relationships.packages.maintainers.pluck(:package_id) })
      end
      ...
      def review_query
        ...
        @bs_requests.where(
          reviews: {
            user_id: user.id,
            group_id: user.groups.pluck(:id),
            project_id: user.relationships.projects.maintainers.pluck(:project_id),
            package_id: user.relationships.packages.maintainers.pluck(:package_id)
          }
        )
      end
