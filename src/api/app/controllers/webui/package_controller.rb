class Webui::PackageController < Webui::WebuiController
  ...
  before_action :require_login, except: %i[show index branch_diff_info
                                           ...
                                           devel_project buildresult rpmlint_result rpmlint_log rpmlint_beta rpmlint_beta_accordion files]
  ...
  before_action :set_project, only: %i[show edit update index users requests statistics revisions
                                       ...
                                       buildresult rpmlint_result rpmlint_log rpmlint_beta rpmlint_beta_accordion files]
  ...
  before_action :require_package, only: %i[edit update show requests statistics revisions
                                           ...
                                           save_person save_group remove_role view_file
                                           buildresult rpmlint_result rpmlint_log rpmlint_beta rpmlint_beta_accordion files users]
  # rubocop:enable Rails/LexicallyScopedActionFilter
  before_action :lints_list, only: %i[rpmlint_beta rpmlint_beta_accordion]
  ...
  def rpmlint_beta
    render partial: 'webui/package/beta/rpmlint_beta', locals: { lints_list: @results, badness: @badness, errors: @errors, warnings: @warnings, info: @info, project: @project, package_name: @package_name, repository: @repository, architecture: @architecture }
  end

  def rpmlint_beta_accordion
    render partial: 'webui/package/beta/rpmlint_beta_accordion', locals: { lints_list: @results, badness: @badness, errors: @errors, warnings: @warnings, info: @info, project: @project, package_name: @package_name, repository: @repository, architecture: @architecture }
  end
  ...
  def lints_list
    ...
    @warnings = 0
    @info = 0
    if (@buildresult = Buildresult.find_hashed(project: @project.to_param, package: @package.to_param, view: 'status'))
      @buildresult.elements('result') do |result|
        if result.value('repository') != 'images' &&
           result.value('status') && result.value('status').value('code') != 'excluded'
          begin
            ...
            if rpmlint_log_file.present?
              ...
              @badness = max_badness if max_badness > @badness if parsed.badness.present?
