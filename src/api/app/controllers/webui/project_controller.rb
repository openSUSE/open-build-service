class Webui::ProjectController < Webui::WebuiController
  ...
  def maintained_projects
    @maintained_projects = []
    @project.maintained_projects.each do |maintained_project|
       @maintained_projects << maintained_project.project.name
    end
  end
  ...
  def load_project_info
    ...
    @ipackages = @project.linked_packages.map { |package| [ package.name, package.project.name ] }.sort
    @packages = @project.packages.order(name: :desc).pluck(:name)
    @linking_projects = @project.find_linking_projects.map { |p| p.name }
  ...
  def find_maintenance_infos
    ...
    if @is_maintenance_project
      ...
      @maintained_projects = []
      @project.maintained_projects.each do |mp|
