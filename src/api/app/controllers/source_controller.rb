...
class SourceController < ApplicationController
  ...
  before_action :require_package, only: [:show_package, :delete_package]
  ...
  # before_action for show_package and delete_package
  # FIXME: This does all kinds of things but it does not set @package...
  def require_package
    # init and validation
    #--------------------
  ...
  def project_command_remove_flag
    ...
    @project.transaction do
      @project.remove_flag(params[:flag], params[:repository], params[:arch])
      @project.store
    end
    render_ok
  end
