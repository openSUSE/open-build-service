...
class SourceController < ApplicationController
  ...
  def index_project_createpatchinfo
    ...
    issues = Array.new()
    pro.db_packages.each do |p|
      # create diff per package
      begin
        answer = Suse::Backend.post("/source/#{CGI.escape(pro.name)}/#{CGI.escape(p.name)}?unified=1&cmd=diff&filelimit=0&expand=1", nil)
    ...
    xml = node.patchinfo(attrs) do |n|
      ...
      node.description ""
      issues.each do |i|
        node.issue({ :tracker => i[:issue_tracker], :id => i[:name] })
      end
    end
