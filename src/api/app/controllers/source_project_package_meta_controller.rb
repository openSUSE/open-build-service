class SourceProjectPackageMetaController < SourceController
  ...
  def show
    ...
    if params.key?(:deleted) || params.key?(:meta) || params.key?(:rev) || params.key?(:view) || pack.nil?
      ...
      # deleted package meta gets also delivered if former existing
      # meta parameter is used as a hack to use back as authorative instead
      path = request.path_info
      path += build_query_from_hash(params, [:deleted, :meta, :rev, :view])
