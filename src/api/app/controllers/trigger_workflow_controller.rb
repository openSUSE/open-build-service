class TriggerWorkflowController < ApplicationController
  ...
  def create_workflow_run
    unless @token.is_a?(Token::Workflow)
      raise Trigger::Errors::InvalidToken,
            'Wrong token type. Please use workflow tokens only.'
    end

    request_headers = request.headers.to_h.keys.filter_map do |k|
