...
class SourcePackageCommandController < SourceController
  ...
  after_action :verify_authorized, except: %i[branch diff fork getprojectservices showlinked linkdiff release servicediff waitservice]
  ...
  def diff
    ...
    project_package.each_pair do |project_name, package_name|
      next if Project.remote_project?(project_name)

      package = Package.get_by_project_and_name(project_name, package_name,
                                                use_source: false,
  ...
  def set_package
    options = { updatepatchinfo: { follow_project_links: false },
                ...
                commitfilelist: { follow_project_links: false, follow_project_scmsync_links: true },
