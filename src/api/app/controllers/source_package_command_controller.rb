...
class SourcePackageCommandController < SourceController
  ...
  before_action :validate_project_name, only: :diff
  # copy: is copying *to* the package
  # undelete: is about to re-create the package
  before_action :set_package, except: %i[copy undelete]
  before_action :validate_package_name, only: %i[copy undelete]
  ...
  def release
    ...
    if params[:target_project]
      ...
      target_repository = target_project.project.repositories.find_by!(name: params[:target_repository])
  ...
  def set_package
    ...
    @package = Package.get_by_project_and_name(@project.name, params[:package], options[params[:cmd]])
  end
