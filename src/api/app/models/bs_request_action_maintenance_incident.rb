class BsRequestActionMaintenanceIncident < BsRequestAction
  ...
  def _merge_pkg_into_maintenance_incident(incident_project)
    ...
    if kinds.include?('patchinfo')
      ...
      branch_params[:missingok] = 1 unless Package.exists_by_project_and_name(branch_params[:project], package_name, follow_project_links: true, allow_remote_packages: true)
