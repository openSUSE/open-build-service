class Workflow::Step::SubmitRequest < Workflow::Step
  ...
  def submit_package
    ...
    SCMStatusReporter.new(event_payload: { number: @bs_request.number, state: @bs_request.state }, event_subscription_payload: workflow_run.payload, scm_token: @token.scm_token, workflow_run: workflow_run, event_type: 'Event::RequestStatechange').call
    @bs_request
  end
