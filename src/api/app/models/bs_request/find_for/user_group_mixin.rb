  ...
  module FindFor
    module UserGroupMixin
      ...
      def bs_request_actions(obj)
        ...
        if packages_query(obj).present?
          query_string += " OR ((target_project, target_package) IN (#{packages_query(obj)}))"
        end
      ...
      def reviews(obj, review_states)
        ...
        if obj.is_a?(::User) && usergroups_query(obj).present?
          query_string += " OR by_group IN (#{usergroups_query(obj)})"
        end
      ...
      def packages_query(obj)
        ...
        @packages_query = projects_and_packages.map do |project, package|
          "(#{quote(project)},#{quote(package)})"
        end.join(',')
      end
    end
  end
