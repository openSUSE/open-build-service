class Decorators::Notification::Event::ClearedDecision < Decorators::Notification::Common
  ...
  def notifiable_link_path
    ...
    link_for_reportables(notification, reportable)
  end
  ...
  # be adapted
  def link_for_reportables(notification, reportable)
    return '#' unless reportable

    case notification.event_payload['reportable_type']
    when 'Comment'
      link_for_commentables_on_reportables(commentable: reportable.commentable, notification: notification)
    when 'Package'
  ...
  def link_for_commentables_on_reportables(commentable:, notification:)
    case commentable
    when BsRequest
