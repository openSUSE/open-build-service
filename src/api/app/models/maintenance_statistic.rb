  ...
  def self.create_objects(project, request)
    ...
    if request
      ...
      if request.accepted_at
        result << MaintenanceStatistic.new(type: :release_request_accepted, when: request.accepted_at)
      else
        result << MaintenanceStatistic.new(type: "release_request_#{request.state}", when: request.updated_at)
      end
