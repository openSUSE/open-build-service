...
class BsRequest < ActiveRecord::Base
  ...
  def create_state_notification_event(prefix, additional_notify_parameters = {})
    ...
    case state
    when :accepted
      "#{prefix}AcceptedEvent".constantize.create notify
    when :declined
      "#{prefix}DeclinedEvent".constantize.create notify
    when :revoked
      "#{prefix}RevokedEvent".constantize.create notify
    end
  end
  ...
  def send_state_change
    RequestStatechangeEvent.create(self.notify_parameters) if self.state_was.to_s != self.state.to_s
  end
