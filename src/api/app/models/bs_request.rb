...
class BsRequest < ActiveRecord::Base
  ...
  def expand_targets
    ...
    def self.collection(opts)
      ...
      requests = requests.from_source_project(opts[:source_project]) unless opts[:source_project].blank?
      requests = extend_query_for_project(requests, roles, states, review_states, opts[:package], opts[:subprojects], opts[:project]) unless opts[:project].blank?
      requests = extend_query_for_user(opts[:user], requests, roles, review_states, opts[:package], opts[:subprojects], opts[:project])  if opts[:user]
      requests = extend_query_for_group(opts[:group], requests, roles, review_states)  if opts[:group]
