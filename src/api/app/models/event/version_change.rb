module Event
  class VersionChange < Base
    ...
    def create_local_package_version
      ...
      attribute_anitya_distribution = AttribType.find_by_namespace_and_name('OBS', 'AnityaDistribution')

      return if (package.project.attribs.find_by_attrib_type_id(attribute_anitya_distribution.id) || package.attribs.find_by_attrib_type_id(attribute_anitya_distribution.id)).blank?

      PackageVersionLocal.create(version: payload['newversion'], package: package)
    end
  end
end
