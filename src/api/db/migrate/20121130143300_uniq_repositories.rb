class UniqRepositories < ActiveRecord::Migration
  def up
    ...
    self.transaction do
      ...
      Project.find_by_sql("SELECT DISTINCT projects.* FROM repositories AS r1, repositories AS r2 LEFT JOIN projects on r2.db_project_id = projects.id WHERE r1.db_project_id = r2.db_project_id AND r1.name = r2.name and r1.id != r2.id;").each do |prj|
        rep_ids = {}
  ...
  def down
    execute("alter table repositories modify name tinyblob;")
