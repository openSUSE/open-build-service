class SplitReviewReferences < ActiveRecord::Migration[5.1]
  def up
    ...
    add_reference(:reviews, :user, foreign_key: true, type: :int)
    sql = <<-SQL
      UPDATE reviews
    ...
    add_reference(:reviews, :group, foreign_key: true, type: :int)
    sql = <<-SQL
      UPDATE reviews
    ...
    add_reference(:reviews, :project, foreign_key: true, type: :int)
    sql = <<-SQL
      UPDATE reviews
    ...
    add_reference(:reviews, :package, foreign_key: true, type: :int)
    sql = <<-SQL
      UPDATE reviews
      INNER JOIN projects ON projects.name = reviews.by_project
  ...
  def down
    ...
    remove_reference(:reviews, :group, foreign_key: true)
    remove_reference(:reviews, :project, foreign_key: true)
    remove_reference(:reviews, :package, foreign_key: true)
  end
end
