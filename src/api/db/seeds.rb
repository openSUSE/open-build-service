...
# NOTE: armvXel is actually obsolete (because it never exist as official platform), but kept for compatibility reasons. armv7hl is in for compatibility (soft/hard).
%w(aarch64 armv4l armv5l armv6l armv7l armv5el armv6el armv7el armv7hl armv8el hppa i586 i686 ia64 local m68k mips mips32 mips64 ppc ppc64 ppc64p7 ppc64le s390 s390x sparc sparc64 sparc64v sparcv8 sparcv9 sparcv9v x86_64).each do |arch_name|
...
RolesUser.where(user_id: admin.id, role_id: admin_role.id).order(:role_id).first_or_create

puts "Seeding static_permissions table..."
%w(status_message_create set_download_counters download_binaries source_access access global_change_project global_create_project global_change_package global_create_package change_project create_project change_package create_package).each do |sp_title|
...
IssueTracker.where(name: 'bnc').first_or_create(description: 'openSUSE Bugzilla', enable_fetch: true, kind: 'bugzilla', regex: '(?:bnc|BNC|bsc|BSC|boo|BOO)\s*[#:]\s*(\d+)', url: 'https://bugzilla.opensuse.org/', label: 'boo#@@@', show_url: 'https://bugzilla.opensuse.org/show_bug.cgi?id=@@@')
